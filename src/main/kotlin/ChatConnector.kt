...
@UnstableDefault
class ChatConnector(private val scanner: Scanner, private val printWriter: PrintWriter, private val client: Socket): Runnable, ChatHistoryObserver {
    ...
    private fun setUser(name: String) : String{
        if (Users.users.contains(name))
            ...
            ChatHistory.newMessage(ChatMessage(Commands.Say,name, "all","has joined the chat",ChatTime.getChatTime()))
            return name
        }
        return ""
    }
